 ...
 * updating the distance of its neighbors if a shorter path is found. If the
 * target vertex is found, the algorithm can be stopped early.
 *
...
void dijkstra(std::vector<std::vector<std::pair<int, int>>>& graph, int source,
              int target, std::vector<int>& distance,
    ...
    while (!pq.empty()) {         // While there are vertices to process.
        ...
        if (u == target) break;  // Stop early if the target vertex is found.
...
int main() {
    ...
    std::cout << "Enter the target city: ";
    std::cin >> target;

    std::vector<std::vector<std::pair<int, int>>> graph(n);

    std::cout << "Enter the routes (city1 city2 distance):\n";
    for (int i = 0; i < m; i++) {
        int u, v, weight;
        std::cin >> u >> v >> weight;
    ...
    dijkstra(graph, source, target, distance, previous);
