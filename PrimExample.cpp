...
int main() {
    ...
    std::cout << "Enter the number of possible cable connections: ";
    std::cin >> m;

    std::vector<std::vector<std::pair<int, int>>> graph(n);
    std::vector<std::pair<int, std::pair<int, int>>> mst;

    std::cout << "Enter the possible cable connections (house1 house2 cost):\n";
    for (int i = 0; i < m; i++) {
        int u, v, weight;
        std::cin >> u >> v >> weight;
    ...
    std::cout << "Minimum cost to connect all houses:\n";
    int totalCost = 0;
    for (const auto& edge : mst) {
        std::cout << "Connected house " << edge.second.first << " to house "
                  << edge.second.second << " with cost " << edge.first << ".\n";
